		...
		<form action="checkout" method="post">
			<c:if test="${not empty cartProduct}">
					...
					<tbody>
						<c:forEach var="item" items="${cartProduct}">
							<tr>
								...
								<td><a href="delete?id=${item.getCartItemId()}"><fmt:message
											key="cart_delete" /></a></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
				<div class="text-end mt-3">
					<div>
						<strong><fmt:message key="cart_total" />: </strong> <span>${sumCart}</span>
					</div>
					<button type="submit" class="btn btn-success">
						<fmt:message key="cart_checkout" />
					</button>
