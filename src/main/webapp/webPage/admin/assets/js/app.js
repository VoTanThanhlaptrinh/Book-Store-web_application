...
class Portlet {
	...
	initCloser() {
		document.querySelectorAll(this.portletCloser).forEach((element) => {
			element.addEventListener('click', (e) => {
				...
				// Kiểm tra element.closest() có hoạt động không
				const portlet = element.closest(this.portletIdentifier);
				if (!portlet) return;

				const portletParent = portlet.parentElement;
				portlet.remove();

				// Kiểm tra nếu parent không còn con nào thì xoá luôn
	...
	initRefresher() {
		document.querySelectorAll(this.portletRefresher).forEach((element) => {
			element.addEventListener('click', (e) => {
				e.preventDefault();
				...
				let pd = portlet.querySelector('.card-disabled');
